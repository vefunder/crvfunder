...
@given(timedelta=strategy("uint", min_value=0, max_value=2 * YEAR))
def test_emissions_against_expected(alice, gauge_controller, funder, crv20, timedelta):
...
@pytest.mark.skip_coverage
@given(max_emissions=strategy("uint", min_value=0, max_value=500_000 * 10 ** 18))
def test_reach_emissions_max(
    alice, charlie, gauge_controller, factory, CRVFunderLocal, crv20, max_emissions
):
